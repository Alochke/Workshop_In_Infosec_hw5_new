                        ...
                        const struct nf_hook_state *state)
{
    ...
    {
        struct tcphdr *tcp_head = tcp_hdr(skb);
        printk("Local out %pI4:%u to %pI4:%u with SYN %d ACK %d FIN %d RST %d", &iphead->saddr, ntohs(tcp_head->source),
           &iphead->daddr, ntohs(tcp_head->dest), tcph_ead->syn, tcp_head->ack, tcp_head->fin, tcp_head->rst);
